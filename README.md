...
# Sampling parameters
pulse_rate = 50                      # Pulse rate
pulse_width = int(fs / pulse_rate / 2)  # Pulse width
...
# Flat-top sampling: sample & hold (constant value held during the pulse width)
flat_top_signal = np.zeros_like(t)
sampled_signal = message_signal[pulse_train == 1]
sample_times = t[pulse_train == 1]

sample_indices = np.where(pulse_train == 1)[0][::pulse_width]  # Only take one per pulse
for idx in sample_indices:
    flat_top_signal[idx:idx + pulse_width] = message_signal[idx]

# Optional: Low-pass filter to reconstruct
def lowpass_filter(signal, cutoff, fs, order=5):
    nyquist = 0.5 * fs
    normal_cutoff = cutoff / nyquist
    b, a = butter(order, normal_cutoff, btype='low', analog=False)
...
reconstructed_signal = lowpass_filter(flat_top_signal, 10, fs)
...
plt.subplot(4, 1, 3)
plt.plot(t, flat_top_signal, label='Flat-Top Sampled Signal', color='orange')
plt.legend()
plt.grid(True)
...
![image](https://github.com/user-attachments/assets/d1ed4092-a77d-4608-bac8-5cb8c68d595b)
